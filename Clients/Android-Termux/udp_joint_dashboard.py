...
(JointStateBroadcaster.cs). Sirve un dashboard web local con:
  ...
  - Una vista 3D del robot (Three.js, corre en el navegador del celular)
    que calcula la cinemática directa con la MISMA tabla DH y formula de
    Offset/Factor que ya usamos en robot_udp_plot.m (MATLAB).

Pensado para correr en Termux (Android): la GPU/render la hace el
navegador del celular via WebGL, no Termux, asi que no hace falta
instalar roboticstoolbox-python/Swift ni compilar numpy/scipy en el
telefono.

Requiere internet en el celular la primera vez que se abre la pagina,
para bajar three.js desde CDN (cdnjs.cloudflare.com). Si no hay
internet disponible en esa red, avisar y lo resolvemos sirviendo
three.js localmente desde Flask en vez de via CDN.
...
Uso:
    python udp_joint_dashboard.py
    (pregunta la IP/puerto de Unity por consola, igual que los otros scripts)

Despues abrir en el navegador del celular:
    http://127.0.0.1:8080
"""
...
<body>
  ...
  <div id="viewer3d">
    <div class="viewer-hint">arrastrar: rotar &middot; rueda/pellizcar: zoom</div>
  </div>
...
// ---------------------------------------------------------------------
// Camara orbital manual (touch/mouse), sin dependencias extra
// ---------------------------------------------------------------------
...
def main():
    ...
    print(f"Dashboard listo. Abri en el navegador del celular:")
