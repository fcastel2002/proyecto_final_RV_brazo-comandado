...
| `PerformanceMetricsPanel.cs` | Panel estático (`Metrics_Section`, hijo de `InfoPanel_Gripper`) con 4 filas fijas (colisiones, tiempo de operación, cantidad de operaciones, masa agarrada); expone `SetExtraRow` para agregar métricas nuevas clonando una plantilla inactiva solo si hace falta | UI de Métricas de Desempeño |
| `PerformanceMetricsTracker.cs` | En `OnRobot_RG2`, junto a `GripperController`. Detecta la transición agarre→suelta leyendo `GripperController.IsHoldingObject`/`GrabbedMass` (sin modificar ese script) para contar operaciones y medir su duración; recibe notificaciones de `GripperCollisionCounter` para el conteo de colisiones, deduplicando por objeto | Lee `GripperController`, escribe en `PerformanceMetricsPanel` |
| `GripperCollisionCounter.cs` | En `CollisionSensor` (hijo de `OnRobot_RG2`, `BoxCollider` en modo Trigger dimensionado para dedos abiertos, ya que el collider no seguí la apertura/cierre real). Cuenta contactos por trigger (no colisión sólida: el Rigidbody del gripper es Kinematic y `OnCollisionEnter` no está garantizado contra colliders estáticos) contra objetos sin tag "Agarrable"; ignora al propio robot via `Ignore Root` | Notifica a `PerformanceMetricsTracker` |
| `ConnectedClientsPanel.cs` | Panel estático (`Clients_Section`, hijo de `InfoPanel_Gripper`) con título + fila fija "Sin clientes conectados" + plantilla inactiva (`Clients_RowTemplate`); clona una fila por cada `IPEndPoint` activo en `JointStateBroadcaster.GetActiveSubscribers()` y destruye las que quedan obsoletas. Reemplaza la versión anterior, que se auto-instanciaba por completo en runtime (`RuntimeInitializeOnLoadMethod`) y clonaba su plantilla de fila desde `PID_Row_J1` (una fila ajena, en uso real), lo que rompía el layout del Canvas de forma intermitente. Con esto, todo `InfoPanel_Gripper` queda estático salvo la cantidad variable de filas de IP, que ahora clona desde su propia plantilla dedicada, igual que el resto de los paneles | UI de Clientes Conectados |
| `JointStateBroadcaster.cs` | Servidor UDP con suscripción dinámica (puerto `_serverPort`, default 25001): lee `Controller.MechanicalGroup.JointState` y transmite `"q1,q2,q3,q4,q5,q6,gripper\n"` en grados a `_sendHz`. El 7mo campo (`gripper`, 0=cerrado..1=abierto) sale de `Ctrl_OnRobotRG2_Custom.OpeningFraction` (campo `_gripperAnimator`, autoasignado por `FindFirstObjectByType` en `Start()` si se deja vacío en el Inspector; sin gripper en escena se envía 0). Solo telemetría de solo lectura: no participa de la cadena de control (no escribe `JointTarget` ni mueve el robot) | Independiente (consumido por `Clients/Python/udp_robot_viewer.py`, `udp_robot_dashboard.py` y `Clients/MATLAB/robot_udp_plot.m`) |
